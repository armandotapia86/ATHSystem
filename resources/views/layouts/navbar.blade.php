<div class="navbar bg-base-100 shadow-lg">
    ...
    <div class="flex-1 hidden md:flex space-x-4">
        <a href="{{ route('home') }}" class="btn btn-ghost btn-sm">ATHSystem</a>
        <a href="{{ route('servicios') }}" class="btn btn-ghost btn-sm">Servicios</a>
        <a href="{{ route('productos.index') }}" class="btn btn-ghost btn-sm">Productos</a>
        <a href="{{ route('acerca-de-nosotros') }}" class="btn btn-ghost btn-sm">Acerca de nosotros</a>
        <a href="{{ route('carrito') }}" class="relative btn btn-ghost btn-sm">
            <!-- Ícono del carrito -->
            <svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-7 text-white-700" fill="none" viewBox="0 0 24 24"
                stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round"
                    d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13l-2 4m2-4l1.5-7h11l1.5 7M17 13l2 4m-5.5 0h-5m6 0a1.5 1.5 0 11-3 0m-6 0a1.5 1.5 0 11-3 0" />
            </svg>
            <!-- Contador de productos -->
            <span class="absolute -top-2 -right-2 bg-amber-500 text-white text-xs font-bold rounded-full px-2">
                {{ session('carrito') ? count(session('carrito')) : 0 }}
            </span>
        </a>
    </div>
